...
if ( ! function_exists( 'hitandrun_setup' ) ) :
	...
	function hitandrun_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'hitandrun_custom_background_args',
				array(
...
function hitandrun_scripts() {
	...
	wp_enqueue_style( 'hitandrun-style', get_stylesheet_uri(), array(), '1.0.0' );
	...
	wp_enqueue_script( 'hitandrun-js', get_stylesheet_directory_uri() . '/assets/js/hitandrun.min.js', array(), '1.0.0', true );

	$l10n = array(
		'img_dir' => wp_upload_dir(),
	);
	wp_localize_script( 'hitandrun-js', 'WONKASOFT_HELPER', $l10n );
